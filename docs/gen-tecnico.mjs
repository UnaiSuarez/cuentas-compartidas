...
const buildTecnico = async () => {
  ...
  const infoItems = [
    ...
    ['Tecnologia principal:', 'React 19 + Firebase + Tailwind CSS v4'],
    ['Idioma:', 'Espanol'],
  ];
  ...
  y = code(doc, y, [
    ...
    '  avatar:    string,       // ID del avatar (numero de 0 a 14)',
    '  groupId:   string|null,  // ID del grupo al que pertenece',
    '  createdAt: timestamp,    // Fecha y hora de creacion',
    '  updatedAt: timestamp,    // Fecha y hora de ultima modificacion',
    '  settings: {',
  ...
  y = code(doc, y, [
    ...
    '  type:      "user" | "system",',
    '  metadata?: {                  // Solo para mensajes de sistema',
    '    paymentFrom:   string,      // UID del que paga',
    '    paymentTo:     string,      // UID del que cobra',
  ...
  y = code(doc, y, [
    ...
    '    const pagadoPor = tx.paidBy || tx.pagado_por',
    '    const participantes = tx.splitAmong || tx.dividido_entre',
    '',
    ...
    '    if (balances[pagadoPor] !== undefined)',
    '      balances[pagadoPor] += monto        // Adelanto el total',
    '',
    '    participantes.forEach(uid => {',
    '      if (balances[uid] !== undefined)',
    '        balances[uid] -= partePorPersona  // Debe su parte',
    '    })',
    '  })',
    '  return balances',
    '}',
  ...
  y = code(doc, y, [
    ...
    '',
    '    // Usuario: solo puede leer/escribir su propio documento',
    '    match /users/{userId} {',
