...
export function useAuth() {
  ...
  /**
   * El usuario abandona una sala.
   * Si es admin y quedan miembros → el siguiente hereda el rol de admin.
   * Si es el único miembro → la sala queda vacía (sin borrar datos históricos).
   */
  async function leaveGroup(uid, groupId) {
    ...
    try {
      ...
      if (isAdmin && remaining.length > 0) {
        groupUpdate.createdBy = remaining[0]
      }
      ...
      localStorage.removeItem(`activeGroup_${uid}`)
    } catch (e) {
      setError('Error al salir de la sala: ' + e.message)
      throw e
    } finally {
      setLoading(false)
    }
  ...
  /**
   * El admin expulsa a un miembro de la sala.
   */
  async function removeMember(adminUid, targetUid, groupId) {
    ...
    try {
      const groupSnap = await getDoc(doc(db, 'groups', groupId))
      if (!groupSnap.exists()) throw new Error('Sala no encontrada')
      if (groupSnap.data().createdBy !== adminUid) throw new Error('Sin permisos de admin')
      ...
      setError('Error al eliminar miembro: ' + e.message)
      throw e
    } finally {
      setLoading(false)
    }
